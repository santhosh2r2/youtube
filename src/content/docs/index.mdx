---
title: My Blog site
description: The aim of this website is to share to the community what I have learned during the past years
template: splash
layout: ../../layouts/BaseLayout.astro
editUrl: false
hero:
  tagline: Welcome, here I blog about tech stuff pertaining to Automation and IT fields. This site serves as a companion website of my youtube channel
  image:
    file: ../../assets/pic.jpg
  title: Hi
  actions:
    - text: Blog
      link: /youtube/blog/
      icon: right-arrow
      variant: primary
    - text: Youtube
      link: https://www.youtube.com/channel/UCR_Fuegjqal0Fvy6En2Bs3Q?sub_confirmation=1
      icon: youtube
    # - text: LinkedIn
    #   link: https://www.linkedin.com/in/santhosh-balaji-ramesh/
    #   icon: linkedin
    # - text: Contact me
    #   link: mailto:santhosh.mwpa@gmail.com
    #   icon: email
---
import { Card, CardGrid, LinkCard, Icon , Aside} from '@astrojs/starlight/components';

## Explore

<CardGrid stagger>
  {/* <Card title="Projects" icon="seti:favicon" >
    To view projects, click [here](/youtube/projects/).
  </Card> */}
  <Card title="Tutorials" icon="rocket" >
		Checkout tutorials [here](/youtube/tutorial/).
	</Card>
  <Card title="Tips and Tricks" icon="list-format" >
		Checkout all tips and tricks [here](/youtube/general/tips-and-tricks/).
	</Card>
  <Card title="Boilerplates" icon="document" >
		Explore boilerplates, starter-kits and much more, [here](/youtube/general/boilerplate/).
	</Card>
  <Card title="Installations" icon="setting" >
		To view installation guides, click [here](/youtube/general/install/).
	</Card>
</CardGrid>

<Aside type="tip" title="Note">
Bookmark this page for quick-reference
</Aside>

[![Built with Starlight](https://astro.badg.es/v2/built-with-starlight/tiny.svg)](https://starlight.astro.build)

```mermaid
gitGraph
    commit
    commit
    commit
    commit
    branch develop
    checkout develop
    commit
    commit
    checkout main
    merge develop
    commit
    branch test
    checkout test
    commit
```

```mermaid
flowchart TD
    A[Install SQL Server with `sa` user] --> B[Create a user `DSEEvoris` with sysadmin role]
    B --> C[Disable `sa` user]
    C --> D[Delete Windows user used during installation]
    B --> E[Create `operator` user]
    E --> F[Assign operator user with `db_roles`]
```